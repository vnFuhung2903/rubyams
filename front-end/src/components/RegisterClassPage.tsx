...
export default function RegisterClassPage() {
  ...
  const handleDelete = () => {
    setClasses(classes.filter((_, idx) => !selected.includes(idx)));
    setSelected([]);
  };
  ...
  const handleAdd = () => {
    ...
    setClasses([
      ...classes,
      { code: input.trim().toUpperCase(), name: "Lớp mới", date: "4/1/2022", status: "Thành công", credits: 2 }
    ]);
    setInput("");
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert("Gửi đăng ký lớp thành công!");
  };
  ...
  return (
    <div className="container" style={{ background: "#fff", borderRadius: 16, margin: "32px auto", padding: 32, boxShadow: "0 2px 12px #25324e0a" }}>
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ textAlign: "center", fontWeight: 600, marginBottom: 8, fontSize: 17 }}>Bảng đăng ký lớp kỳ 20221 của sinh viên 20202849</div>
